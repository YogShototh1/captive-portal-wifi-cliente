...
:if ([:tostr $cdfase] != "2") do={
  ...
  :local arq ""
  :foreach f in=[/file find where name~"setup-varejo.rsc"] do={ :set arq [/file get $f name] }
  :if ([:len $arq] = 0) do={
    :put "!! Nao achei o setup-varejo.rsc em /file. Reenvie pelo Winbox (Files) e rode de novo."
    :log warning "cdsetup ABORTOU: setup-varejo.rsc nao esta em /file"
