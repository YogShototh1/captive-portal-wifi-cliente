...
function mountScrollWorld(container, config) {
  ...
  function read() {
    ...
    for (let i = 0; i < NSEG; i++) {
      const s = SEGMENTS[i];
      // 3 telas de antecedencia (era 1.6): com menos, o conector so comecava
      // a baixar quando ja estava quase na tela e a cena passava mostrando o
      // poster com zoom em vez do voo.
      if (y > s.start - 3 * vh && y < s.end + 2 * vh) loadClip(s);
  ...
  layout();
  // Adianta os primeiros clipes: a 1a emenda chega antes de qualquer
  // prefetch por scroll ter acontecido.
  SEGMENTS.slice(0, 3).forEach(loadClip);
  requestAnimationFrame(raf);
